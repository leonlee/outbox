...
| 配置项 | 值 | 原因 |
|--------|-----|------|
| `WriterHook` | `NOOP`（默认值） | 禁用热路径，避免双路径乱序 |
| `OutboxPoller` | 单节点 | 防止跨节点 Claim 交错 |
| `workerCount` | `1` | 顺序分发保持轮询顺序 |

Poller 按 `ORDER BY created_at` 顺序读取事件。单个分发线程依次处理，
保证同一聚合根的事件按插入顺序投递。

### 19.2 为什么是仅 Poller 模式

热路径 + 冷路径的双通道架构在一般情况下难以保证顺序——同一聚合根的事件
可能通过不同路径以不可预测的顺序到达。禁用热路径（`WriterHook.NOOP`）
确保所有事件都经由 Poller 流入，而 Poller 按 DB 插入顺序读取。

### 19.3 重试会打破顺序

如果事件 A 失败并被标记为 RETRY（指数退避），其 `available_at` 被设为未来时间。
此时同一聚合根的事件 B（插入时间晚于 A）满足 `available_at <= now`，会先于 A
的重试被轮询并投递——打破聚合根维度的有序性。

**缓解方案：** 设置 `maxAttempts(1)`，使失败事件直接进入 DEAD 而不重试。
通过 `DeadEventManager` 检查问题并在修复后手动重放。
